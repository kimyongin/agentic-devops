...
name: run
description: >
  AgenticDevOps 파이프라인 오케스트레이터. /agentic-devops:run [요구사항] 또는
  /agentic-devops:run status / design / task / code 로 호출.
...
3. Gate Keeper 검증 보고서 요약을 출력한 후, **AskUserQuestion 툴**로 승인을 요청한다:
...
- header: `"게이트 판정"`
- options:
  - label `"승인"` / description `"다음 단계를 진행합니다"`
  - label `"반려"` / description `"해당 에이전트를 재작업합니다"`
  - label `"수정 후 재검증"` / description `"수정 완료 후 Gate Keeper를 재실행합니다"`
...
| local-runner SKIP 보고 | AskUserQuestion으로 최종 승인 요청 |
| local-runner 실행 중 보고 | 테스트 안내 후 AskUserQuestion으로 결과 요청 |
| local-runner 실패 보고 | 원인 보고 후 AskUserQuestion으로 처리 방법 요청 |

**SKIP 보고 시** AskUserQuestion 호출:
- question: `"Docker 환경이 없어 로컬 테스트를 건너뜁니다. 파이프라인을 완료하시겠습니까?"`
- header: `"최종 승인"`
- options:
  - label `"파이프라인 완료"` / description `"로컬 테스트 없이 완료합니다"`
  - label `"반려"` / description `"파이프라인을 완료하지 않습니다"`

**실행 중 보고 시** (사용자 수동 테스트 완료 후) AskUserQuestion 호출:
- question: `"로컬 테스트 결과를 알려주세요"`
- header: `"로컬 테스트"`
- options:
  - label `"테스트 완료"` / description `"파이프라인을 완료합니다"`
  - label `"이슈 발견 - 코드 수정"` / description `"이슈를 설명하면 code-writer를 재스폰합니다"`
  - label `"이슈 있지만 완료"` / description `"이슈를 인지하고 파이프라인을 완료합니다"`

**실패 보고 시** AskUserQuestion 호출:
- question: `"로컬 테스트 환경 구성에 실패했습니다. 어떻게 처리하시겠습니까?"`
- header: `"테스트 실패"`
- options:
  - label `"코드 수정"` / description `"code-writer를 재스폰하여 실패 원인을 수정합니다"`
  - label `"파이프라인 완료"` / description `"로컬 테스트 없이 파이프라인을 완료합니다"`
...
3. 사용자에게 영향 범위를 보고한 후, **AskUserQuestion 툴**로 확인을 요청한다:
   - question: `"[영향 범위 설명]. 변경을 진행하시겠습니까?"`
   - header: `"변경 요청"`
   - options:
     - label `"승인 - 변경 진행"` / description `"영향받는 산출물을 DRAFT로 되돌리고 해당 에이전트를 재스폰합니다"`
     - label `"반려 - 현재 유지"` / description `"변경 없이 현재 범위로 진행합니다"`
