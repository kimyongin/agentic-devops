...
> **CODE 단계 흐름**: code-writer 완료 → gate-keeper(프로세스 검증) → 사람 승인 → code-reviewer(코드 품질 검증) → 사람 승인 → local-runner(로컬 테스트 환경) → 사람이 테스트 후 최종 승인 → 파이프라인 완료.
...
| 반려 시점 | 재스폰 대상 | 이후 흐름 |
|-----------|------------|----------|
| gate-keeper 검증 후 반려 | `code-writer` | code-writer → gate-keeper → 사람 승인 |
| code-reviewer 보고 후 반려 | `code-writer` (수정 사항 포함) | code-writer → gate-keeper → 사람 승인 → code-reviewer |
...
**타임스탬프 획득**: HISTORY에 시각을 기록할 때는 반드시 `bash scripts/now.sh` 명령을 실행하여 현재 시각을 얻는다. LLM이 추측한 시각을 사용하지 않는다.
...
code-reviewer 보고서를 사람이 승인한 후, 로컬 테스트 환경을 제공한다:
