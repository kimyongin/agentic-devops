...
5. PRD의 범위와 도메인 구조를 분석하여, 단일 DESIGN으로 다루기 어려운 경우 서브도메인별 분할을 판단한다.
...
### 설계 분할 기준(DESIGN Splitting)

하나의 PRD에서 복수의 DESIGN 문서를 생성할 수 있다. 아래 기준에 따라 분할 여부를 판단한다:

- 바운디드 컨텍스트가 2개 이상이고, 각각의 도메인 모델·계약이 독립적인 경우
- 단일 문서로 작성하면 유즈케이스가 10개를 초과하여 문서 관리가 어려운 경우
- 서브도메인 간 개발 순서가 달라 단계적 승인이 필요한 경우

분할 시 각 DESIGN은 동일한 PRD를 참조하되, 담당 서브도메인을 slug에 반영한다.
(예: DESIGN-0001-auth-domain, DESIGN-0002-order-domain)
...
PRD `6) 기술 스택` 섹션을 참조한다. 아래에는 PRD에서 변경되거나 추가 확정된 항목만 기록한다:
...
> 구체적인 테스트 일정/순서는 작업 계획(TASK)에서, 작성 방법은 `code-writer` 스킬에서 다룬다.
