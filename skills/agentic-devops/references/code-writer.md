...
- 파이프라인: PRD → DESIGN → TASK → **CODE** → PR → RELEASE. 승인된 TASK를 입력으로 받는다.
...
- 브랜치 네이밍: `feature/TASK-xxxx-설명`, `fix/TASK-xxxx-설명`, `refactor/TASK-xxxx-설명`
- 상세 브랜칭/머지 전략, 머지 규칙, 충돌 해결은 `pr-manager.md` 참조.
...
CI 게이트 기준 및 체크리스트는 `pr-manager.md` 참조.

---

## PR 템플릿

PR 작성 시 사용할 템플릿은 `pr-manager.md` 참조.
