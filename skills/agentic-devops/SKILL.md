...
description: >
  ...
  code-reviewer, local-runner, gate-keeper)에 preload되어 공통 규칙을 주입한다.
...
> code-writer, code-reviewer, local-runner, gate-keeper)를 위한 **공통 규칙집**이다.
...
```
PRD(DRAFT) →[승인]→ DESIGN(DRAFT) →[승인]→ TASK(DRAFT) →[승인]→ CODE →[승인]→ 완료
...
| REVIEW | `REVIEW-xxxx-slug` | code-writer 변경 파일 | `docs/ai-devops/REVIEW/` |
| HISTORY | `RUN-xxxx-slug` | 파이프라인 실행 | `docs/ai-devops/HISTORY/` |

> - `xxxx`는 **산출물 유형별 독립 순번**(0001, 0002, ...)이며, `slug`는 소문자 kebab-case로 핵심 주제를 요약한다.
> - 예: `PRD-0001-user-auth`, `DESIGN-0001-auth-domain`, `TASK-0001-login-api`, `REVIEW-0001-login-api`, `RUN-0001-user-auth`
...
| TASK : CODE | 1 : 1 | 1 TASK = 1 구현 단위 |
| TASK : REVIEW | 1 : 1 | 하나의 TASK 구현에 대한 코드 리뷰 (버전으로 재리뷰 추적) |
...
- **Owner(사람)**: PRD 작성/승인, 설계 승인, 작업 계획 승인, 코드 승인. Gate Keeper/code-reviewer 보고서를 참고하여 최종 승인/반려 결정
- **AI Writer(도구)**: 가이드 기반 산출물 생성, 변경 제안, 보고
- **Gate Keeper(도구)**: 산출물 검증, 프로세스 일관성 검증, 게이트 판정 보고
- **code-reviewer(도구)**: CODE 단계에서 AI 생성 코드를 교차 검증하고 리뷰 보고서를 작성
- **local-runner(도구)**: REVIEW 승인 후 docker compose로 로컬 테스트 환경을 구성하고 접속 정보를 보고
...
CODE 단계의 세부 흐름(gate-keeper(CODE) → code-reviewer → gate-keeper(REVIEW) → local-runner)은 `run` 스킬에서 정의한다.
...
- **코드 변경**: 작업 범위 내 구현 수정 → 코드 재검증 및 승인만으로 반영 가능
