...
`agentic-devops` 파이프라인 규칙과 `prd-writer` 작성 가이드가 컨텍스트에 사전 로드되어 있습니다.
...
**모드 2 — 검토**: 오케스트레이터가 작성 완료된 템플릿 파일 경로와 함께 "검토"를 지시한 경우.
`prd-writer` 가이드의 `모드 2: 검토 모드` 절차에 따라 내용을 검토하고 최종 PRD를 확정한 후 보고서를 제출하세요.
