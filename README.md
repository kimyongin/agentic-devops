...
아래 표는 참고용 예시이며, **실제 기술 스택은 PRD의 `6) 기술 스택` 섹션에서 사용자가 직접 결정한다.** PRD에 기술 스택이 명시되어 있으면 해당 내용을 따른다.
...
승인
  ...
  → gate-keeper 에이전트 스폰 → REVIEW 검증 → 사람 승인
...
code-reviewer 에이전트 종료
  └─ SubagentStop hook → subagent-stop-handler.js
        └─ "code-reviewer 완료, gate-keeper 실행 필요 (REVIEW 검증)"
...
### 대화가 끊겼을 때 어떻게 이어서 하나요?

`/agentic-devops:run`을 다시 호출하면 된다. 오케스트레이터가 `docs/ai-devops/` 디렉터리를 스캔하여 각 산출물의 상태(DRAFT/APPROVED)를 확인하고, 중단된 지점부터 자동으로 이어간다. 모든 파이프라인 상태가 파일 시스템에 영속화되어 있으므로 대화가 바뀌어도 진행 상황이 유지된다.

### 반려하면 어떻게 되나요?

승인 요청에서 "반려"를 선택하면 해당 단계의 에이전트를 다시 스폰하여 재작업한다. 반려 사유는 HISTORY 파일에 기록되며, 재작업된 산출물은 다시 Gate Keeper 검증 → 사람 승인 흐름을 거친다.
